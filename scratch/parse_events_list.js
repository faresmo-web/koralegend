...
async function inspect() {
    ...
    try {
        ...
        console.log('--- Printing details of each LI inside events ---');
        $('.timeline.events ul li').each((i, el) => {
            const className = $(el).attr('class') || '';
            const html = $(el).html().trim().replace(/\s+/g, ' ');
            console.log(`LI ${i}: class="${className}"`);
