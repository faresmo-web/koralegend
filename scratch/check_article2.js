...
// Use a known article URL directly
...
async function main() {
    ...
    if (d.body !== undefined) {
        ...
        if (typeof d.body === 'string') {
            console.log('body (first 600):', d.body.slice(0, 600));
        } else if (Array.isArray(d.body)) {
            console.log('body array length:', d.body.length);
            d.body.slice(0, 3).forEach((b, i) => {
                console.log(`body[${i}]:`, JSON.stringify(b).slice(0, 300));
            });
        } else {
            console.log('body:', JSON.stringify(d.body).slice(0, 600));
        }
    }

    // content
    if (d.content !== undefined) {
        console.log('\ncontent type:', typeof d.content);
        if (typeof d.content === 'string') console.log('content (first 600):', d.content.slice(0, 600));
    }

    // richText or text
    ['richText', 'text', 'html', 'articleBody', 'description'].forEach(k => {
        if (d[k]) console.log(`\n${k}:`, String(d[k]).slice(0, 300));
    });
}
