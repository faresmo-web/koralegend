...
(async () => {
    try {
        ...
        const url = `${BASE_URL}/_next/data/${buildId}/%D9%83%D8%B1%D8%A9-%D8%A7%D9%84%D9%82%D8%AF%D9%85/%D9%81%D8%B1%D9%8A%D9%82/${encodeURIComponent(teamName)}/${teamId}.json`;
        const r = await axios.get(url, { headers: HEADERS, timeout: 15000 });
        const data = r.data?.pageProps?.data || {};
        
        const news = data.latestNews?.news?.cards || data.latestNews?.cards || [];
        console.log('Total news cards:', news.length);
        if (news.length > 0) {
            console.log('Sample news card:', JSON.stringify(news[0], null, 2));
        }
    } catch (e) {
        console.error(e);
    }
